...
# содержит методы для хранения и обработки данных об авторах
    ...
    #добавление автора в таблицу authors
        ...
        self._execute_query(query, name_author)
        author_id: int = self._get_int(" SELECT last_insert_rowid() FROM authors")
        return Author(author_id, name_author)

    def get_authors(self) -> list[Author]:
    # получение списка всех авторов
        query: str = " SELECT author_id, name_author FROM authors "
        authors: list = self._execute_get_data(query)
    ...
    # проверка наличия автора в базе по переданному id
        ...
        res: int = self._get_int(query, author_id)
        if res == 1:
            return True
        return False
    ...
    def check_name_author(self, name_author: str) -> bool:
     # проверка наличия автора в базе по имени
        ...
        res: int = self._get_int(query, name_author)
        if res == 1:
            return True
        return False
...
# содержит методы для хранения и обработки данных о жанрах
    ...
    #добавление жанра в таблицу genres
        ...
        self._execute_query(query, name_genre)
        genre_id: int = self._get_int(" SELECT last_insert_rowid() FROM genres")
        return Genre(genre_id, name_genre)

    def get_genres(self) -> list[Genre]:
    # получение списка всех авторах
        query: str = " SELECT genre_id, name_genre FROM genres "
        genres: list = self._execute_get_data(query)
    ...
    # проверка наличия жанра в базе по переданному id
        ...
        res: int = self._get_int(query, genre_id)
        if res == 1:
            return True
        return False
    ...
    def check_name_genre(self, name_genre: str) -> bool:
     # проверка наличия жанра в базе по названию
        ...
        res: int = self._get_int(query, name_genre)
        if res == 1:
            return True
        return False
...
# содержит методы для хранения и обработки данных о книгах
    ...
    #добавление книги в таблицу books
        ...
        self._execute_query(query, title, author_id, genre_id)
        book_id: int = self._get_int(" SELECT last_insert_rowid() FROM books")
        b_query: str = '''
            ...
            WHERE book_id = ?
        '''
        books = self._execute_get_data(b_query, book_id)
    ...
    # получение списка всех книг
        query: str = '''
            ...
            JOIN genres g ON b.genre_id = g.genre_id
        '''
        books: list = self._execute_get_data(query)
    ...
    # отметка книги как прочитанной
        ...
        res = self._execute_get_data(" SELECT * FROM books WHERE book_id = ? ", book_id)
    ...
    # поиск книг по названию, автору или жанру
        ...
        books: list = self._execute_get_data(query, param)
        books_list: list[BookInfo] = [BookInfo(*row) for row in books]
        return books_list
    ...
    # проверка наличия книги в базе по переданному id
        ...
        res: int = self._get_int(query, book_id)
        if res == 1:
            return True
        return False
